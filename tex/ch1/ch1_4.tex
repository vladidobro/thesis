...
\todocite{Visbible}
Všechny magneto-optické (MO) jevy lze v principu vysvětlit závislostí optických parametrů na magnetickém stavu\todocite{Silber}.
...
Z termodynamických úvah plynou Onsagerovy relace reciprocity\todocite{Onsager} pro $\varepsilon(\vec{M})$
\begin{equation}
...
\label{eqn:MO-tenzory}
    \varepsilon_{ij}(\vec{M})&=\varepsilon^0_{ij}
...
\end{align}
kde jsme explicitně uvedli první dva řády, které definují \emph{lineární MO tenzor} $\mathbb{K}$ a \emph{kvadratický MO tenzor} $\mathbb{G}$. \todocite{Visbible}
Vyšší řády se většinou zanedbávají.
Je dobré ale mít na paměti, že zakončením rozvoje na určitém řádu nejen snižujeme přesnost, ale také uměle zvyšujeme symetrii $\varepsilon(\M)$ \todocite{Silber}.
...
MO tenzory $\mathbb{K}$ a $\mathbb{G}$ nedokáží popsat šesterečně symetrický Voigtův jev\todo{odkaz na kapitolu}, proto je třeba mít se na pozoru a v případě takového kvalitativního důkazu do rozvoje přidat další členy.
...
Tvar $\mathbb{K}$ a $\mathbb{G}$ pro všechny krystalografické třídy je uveden v \todocite{Visbible}.
...
Pro kubický materiál platí (používáme 2-indexovou notaci jako \todocite{Hamrlova})
\begin{equation}
    \begin{pmatrix}
...
\begin{align}
    ...
    \end{pmatrix}
\end{align}
kde $G_s=G_{11}-G_{12}+2G_{44}$.\todo{overit}
...
Uvedený přístup není možné použít v případě, že osvětlované místo vzorku není tvořené homogenním $\vec{M}$, ale je tvořené více doménami, ve kterých se liší.
